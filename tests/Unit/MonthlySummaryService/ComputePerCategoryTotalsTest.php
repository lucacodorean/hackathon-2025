...
class ComputePerCategoryTotalsTest extends TestCase
{
    public function testComputeTotalPerCategory(): void
    {
        ...
        $rawData = [
            ['category_name' => 'Groceries', 'SUM(amount_cents)' => 15.00],
            ['category_name' => 'Transport', 'SUM(amount_cents)' =>  5.00],
        ];
        ...
        $service->expects($this->once())
            ->method('formatData')
            ->with($rawData, $expectedGrandTotal, 'SUM')
            ->willReturn($expectedFormatted);

        $actual = $service->computePerCategoryTotals($user, $year, $month);
